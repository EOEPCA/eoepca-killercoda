...
```
eodag list --platform SENTINEL-2 --no-fetch
```{{exec}}

### Filter by Processing Level

List products at a specific processing level:

```
eodag list --processingLevel L1 --no-fetch
```{{exec}}
...
```
cat /tmp/earth_search_products.json | head -50
```{{exec}}
