...
What we can do is to create a new dataset for AI traning, which can be done via the Gitlab by [creating a new project]({{TRAFFIC_HOST1_8080}}/projects/new#blank_project). We give to it the `wine-dataset` name, and we uncheck "Initialize this project with a README". After the creation is complete, you can go to the [project Settings/General]({{TRAFFIC_HOST1_8080}}/root/wine-dataset/edit), add `sharinghub:dataset` to the "Project Topics" and save the changes.
