...
if [[ -e /tmp/assets/localdns ]]; then
  ...
  WEBSITES="auth.eoepca.local opa.eoepca.local identity-api.eoepca.local nginx.eoepca.local"
...
if [[ -e /tmp/assets/ignoreresrequests ]]; then
  ...
  echo ...waiting for gatekeeper readiness...  >> /tmp/killercoda_setup.log
