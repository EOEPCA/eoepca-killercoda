...
This tutorial environment uses a proxy to route access to running services. The ingress, the web portal URL and the Keycloak client must all use that public URL, so we set it before running the Deployment Guide script:
...
The shared domain/storage/TLS questions were already answered in the prerequisites step, so this script only asks about settings specific to Application Quality.
...
The script prints the generated OIDC client secret and the generated password for the local admin account. Both are stored in `~/.eoepca/state`{{}}, along with the rest of the configuration. The local admin account owns part of the API's catalogue of analysis tools and pipelines, and we will use it later to call the API from this terminal.
